...
def predict_from_audit(audit: AuditResult, thresholds: dict, risk_enabled: bool = True) -> tuple[str, int, str, str]:
    """Predict relation, hallucination, attribution, and explanation from audit features.

    Low-reliability NLI regions are not treated as confident contradictions. Instead,
    they fall back to unsupported-claim detection when the evidence is relevant but
    not entailing, which avoids excessive abstention on imbalanced RAGTruth labels.
    """
    ...
    if audit.contradiction_score >= contradiction_threshold and audit.max_relevance >= low_relevance:
        return (
            "Insufficient",
            1,
            "Generation-inconsistent",
            "A possible contradiction is detected, but NLI reliability or coverage is too weak for confident contradiction attribution.",
        )
    ...
    if audit.max_relevance >= low_relevance and audit.entailment_score < entailment_threshold:
        if nli_unreliable:
            return (
                ...
                1,
                "Generation-inconsistent",
                "The evidence is related, but high-neutral or low-coverage NLI signals only support an unsupported-claim decision.",
            )
